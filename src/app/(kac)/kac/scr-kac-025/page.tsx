...
function TreeNode({
  ...
  node: { label: string; children?: { label: string; children?: { label: string }[] }[] };
  depth?: number;
  ...
  return (
    <div>
      <div
        ...
        onClick={() => hasChildren && onToggle(node.label)}
      >
        {hasChildren ? (
          <span className="text-[11px] text-[#64748b]">{isCollapsed ? "▶" : "▼"}</span>
        ) : (
...
export default function ScrKac025() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Split layout: Tree (left) + Table (right) — fixed shared height */}
      <div className="flex h-[640px] gap-4">
        {/* Left: Tree panel */}
        <div className="flex w-[260px] flex-shrink-0 flex-col rounded-[18px] border border-[#d7e2ee] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
          <div className="flex-shrink-0 border-b border-[#e7eef5] px-4 py-3">
        ...
        {/* Right: Table panel */}
        <div className="flex flex-1 flex-col rounded-[18px] border border-[#d7e2ee] bg-white shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
          <div className="flex-shrink-0 border-b border-[#e7eef5] px-5 py-3">
